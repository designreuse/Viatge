					...
					<c:choose>
						<c:when test="${!empty serviceListRegister}">
							...
							<table id="footable-res2" class="demo margin-bottom"
								...
								<tbody>
									<c:forEach items="${serviceListRegister}" var="service"
										varStatus="status">
										<tr>
											<td><a
												href="editService?id=${service.id}&cs=${service.idCustomerService}">${service.firstName}</a>
											</td>
											<td>${service.lastName}</td>
											<td><fmt:setLocale value="pt_BR" /> <fmt:formatNumber
													value="${service.price}" type="currency" /></td>
											<td><fmt:formatDate value="${service.dateService}" /></td>
											<td><fmt:formatDate value="${service.seeIn}" /></td>
											<c:choose>
												<c:when test="${true}">
